...
logger.info(f"Using API BASE: {rp}")
...
def to_camel(s):
    ...
    if isinstance(s, str):
        ...
        return dict(
            [(to_camel(k), to_camel(s[k]) if isinstance(s[k], dict) or isinstance(s[k], list) else s[k]) for k in s])
...
def new_project(definition: Dict) -> Dict[str, str]:
    id = registry.create_project(ProjectDef(**to_snake(definition)))
    return {"guid": str(id)}


@router.post("/projects/{project}/datasources", tags=["Project"])
def new_project_datasource(project: str, definition: Dict) -> Dict[str, str]:
...
def new_project_anchor(project: str, definition: Dict) -> Dict[str, str]:
    project_id = registry.get_entity_id(project)
...
def new_project_anchor_feature(project: str, anchor: str, definition: Dict) -> Dict[str, str]:
    project_id = registry.get_entity_id(project)
...
def new_project_derived_feature(project: str, definition: Dict) -> Dict[str, str]:
    project_id = registry.get_entity_id(project)
