...
from registry.models import AnchorDef, AnchorFeatureDef, DerivedFeatureDef, Edge, EntitiesAndRelations, Entity, \
    EntityRef, EntityType, ProjectDef, RelationshipType, SourceDef, _to_uuid
...
class PurviewRegistry(Registry):
    ...
    def _upload_single_entity(self, entity: AtlasEntity):
        try:
            ...
            """
            response = \
            self.purview_client.get_entity(qualifiedName=entity.qualifiedName, typeName=entity.typeName)['entities'][0]
            j = entity.to_json()
            if j["typeName"] == response["typeName"]:
                if j["typeName"] == "Process":
                    if response["attributes"]["qualifiedName"] != j["attributes"]["qualifiedName"]:
                        raise ConflictError(
                            "The requested entity %s conflicts with the existing entity in PurView" % j["attributes"][
                                "qualifiedName"])
                else:
                    if "type" in response['attributes'] and response["typeName"] in (
                    TYPEDEF_ANCHOR_FEATURE, TYPEDEF_DERIVED_FEATURE):
