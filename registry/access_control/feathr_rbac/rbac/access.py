...
from .models.base import AccessType, User, UserAccess, _to_uuid
...
def _get_project_name(id_or_name: Union[str, UUID]):
    try:
        ...
        raise BadRequest(
            f"Project Id {id_or_name} not found in Registry {RBACConfig.FEATHR_REGISTRY_URL}. Please check if the project exists or retry later.")
    except ValueError:
        # It is a name
        pass
    return id_or_name
...
def _get_projects_ids():
    ...
    try:
        response = requests.get(url=f"{RBACConfig.FEATHR_REGISTRY_URL}/projects-ids").content.decode('utf-8')
        rbac.projects_ids = json.loads(response)
    except Exception as e:
        raise BadRequest(f"Failed to get projects ids from Registry {RBACConfig.FEATHR_REGISTRY_URL}, {e}")
